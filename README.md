...
![링크 목록](public/screenshots/links.png)

### 링크 추가/수정

![링크 폼](<public/screenshots/edit links.png>)

### 카테고리 관리

![카테고리 관리](public/screenshots/categories.png)

### 태그 관리

![태그 관리](public/screenshots/tags.png)

### 우선순위

![우선 순위 정렬](public/screenshots/priority.png)

### 즐겨찾기

![즐겨찾기](public/screenshots/favorites.png)
...
- **어려웠던 점**  
  아직 풀스택 개발을 하기에는 개념적으로나 기술적으로 많이 부족하다는 것을 느꼈습니다. 특히 인증, 데이터베이스 설계, RLS 정책, 프론트엔드 상태 관리가 서로 연결되어 있어서 한 부분만 이해해서는 전체 흐름을 안정적으로 만들기 어렵다는 점이 가장 크게 다가왔습니다.

- **개선하고 싶은 점**  
  현재는 MVP 기능 구현에 집중했기 때문에 UI 세부 완성도, 예외 처리, 모바일 화면 대응, 로딩/에러 상태를 더 꼼꼼하게 다듬고 싶습니다. 또한 Supabase 테이블과 정책을 직접 설계하고 검증하는 연습을 더 해서, 단순히 동작하는 수준이 아니라 왜 안전한 구조인지 설명할 수 있는 수준까지 개선하고 싶습니다.

- **새롭게 배운 점**  
  Next.js App Router에서 URL 기반 라우팅을 구성하는 방식, Supabase Auth와 RLS를 사용해 사용자별 데이터를 분리하는 방식, CRUD 기능을 프론트엔드 UI와 연결하는 흐름을 배웠습니다. 또 카테고리나 태그 이름 변경처럼 여러 데이터가 함께 바뀌어야 하는 기능은 클라이언트에서 여러 번 요청하기보다 DB 함수로 처리하는 편이 더 안정적이라는 점도 알게 되었습니다.

- **AI 에이전트를 사용하며 느낀 점**  
  무조건적으로 만들어달라고 하기보다는 기능 하나하나를 구현할 때 내가 명확하게 조건을 요구해야 완성도 있는 결과물을 낼 수 있다는 것을 깨달았습니다. 그러기 위해서는 스스로 개발할 수 있는 능력을 길러야 하고, 혼자 연습해보는 시간을 더 많이 늘려야 한다는 점을 배웠습니다.
